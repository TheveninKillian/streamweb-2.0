<template>
  <div class="wrapper">
    <div class="wrapper-player">
      ...
      <div
        class="twitch-details"
        v-if="live"
        :class="{ active: activeDetails }"
      >
    ...
    <div
      class="wrapper-chat"
      :class="{ active: activeChat, resize: activeDetails }"
    >
...
export default {
  ...
  data() {
    return {
      ...
      callTwitch: () => {
        this.getTwitch.streamInfo().then((data) => {
          if (data.length !== 0) {
            this.twitch.title = data.title;
            this.twitch.game = data.game_name;

            this.getTwitch.gameInfo(data.game_id).then((dataGame) => {
  ...
  beforeCreate() {
    if (localStorage.getItem("token")) {
      ...
      let monthNumber = parseInt(localStorage.getItem("date"));

      if (monthNumber === 9 && month === 0) {
        localStorage.removeItem("token");
      } else if (monthNumber === 10 && month === 1) {
        localStorage.removeItem("token");
      } else if (monthNumber === 11 && month === 2) {
        localStorage.removeItem("token");
      } else if (month >= monthNumber + 3) {
        localStorage.removeItem("token");
      }
    }
  },
  created() {
    if (!localStorage.getItem("token")) {
      if (!this.twitch.isAuthenticated()) {
        ...
        const date = new Date();
        let month = date.getMonth();
        localStorage.setItem("date", month);
